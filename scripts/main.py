...
class ChangelogCIBase:
    ...
    def run(self) -> None:
        ...
        if (
            not self.action_env.event_name == self.PULL_REQUEST_EVENT and
        ...
        if all(
            [
                self.config.comment_changelog or self.action_env.event_name != self.PULL_REQUEST_EVENT,
...
class ChangelogCICommitMessage(ChangelogCIBase):
    ...
    def get_changes_after_last_release(self) -> list:
        ...
        url = (
            f'{self.GITHUB_API_URL}/repos/'
            f'{self.action_env.repository}/commits'
        )
        previous_release_date = self._get_latest_release_date()
...
if __name__ == '__main__':
    ...
    if action_environment.pull_request_branch:
        ...
        checkout_pull_request_branch(
            action_environment.pull_request_branch
        )
        print_message('', message_type='endgroup')
